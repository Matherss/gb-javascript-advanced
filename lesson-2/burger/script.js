...
class Menu {
  ...
  fetchMenu() {
    return {
      ...
      stuffing: [
        ...
        { id: "2", price: 20, calorie: 5, text: "Salat (+20 RUB, +5 kcal)" },
      ...
      topping: [
        {
          ...
          price: 15,
          calorie: 0,
          text: "Seasoning (+15 RUB, +0 kcal)"
        },
        {
          id: "2",
          price: 20,
          calorie: 5,
          text: "Mayonnaise (+20 RUB, +5 kcal)"
        }
      ]
    };
  }
  ...
  render() {
    document
      .getElementById("size")
      .insertAdjacentHTML("afterbegin", this._createHTML(this.size, "option"));
    document
      .getElementById("stuffing")
      .insertAdjacentHTML(
        "afterbegin",
        this._createHTML(this.stuffing, "option")
      );
    document
      .getElementById("topping")
      .insertAdjacentHTML(
        "afterbegin",
        this._createHTML(this.topping, "checkbox")
      );
  }
      ...
      case "checkbox":
        return section
          .map(
            (
              item
            ) => `<div class="topping-cont"><input type="checkbox" name="topping" value="${item.id}" id="topping${item.id}">
                    <label for="topping${item.id}"> ${item.text}</label></div><br>`
          )
          .join("");
    }
  }
...
document
  .getElementById("form")
  .addEventListener("change", (event) => burger.changeIngredient(event.target));
